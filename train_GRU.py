        ...
        with sess.as_default():
            ...
            def train_step(word_batch, pos1_batch, pos2_batch, y_batch, big_num):
                ...
                temp, step, loss, accuracy, summary, l2_loss, final_loss, debug_output_forward, debug_prob = sess.run(
                    [train_op, global_step, m.total_loss, m.accuracy, merged_summary, m.l2_loss, m.final_loss, m.output_forward, m.prob],
                    feed_dict)
                ...
                if step % 50 == 0:
                    ...
                    print('\n')
                    print('step ' + str(step) + ' with loss ' + str(loss) + ' acc ' + str(acc) + '\n')
                    label_not_NA_num = 0
                    for i in y_batch :
                        if i[0] != 1 :
                            label_not_NA_num += 1
