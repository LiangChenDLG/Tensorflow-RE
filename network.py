...
class GRU:
    def __init__(self, is_training, word_embeddings, settings):
        ...
        self.inputs_backward = inputs_backward
        ...
        output_forward = tf.reshape(tf.concat(outputs_forward, 1), [total_num, num_steps, gru_size], name='output_forward')
        output_backward = tf.reverse(tf.reshape(tf.concat(outputs_backward, 1), [total_num, num_steps, gru_size]), 
					[1], name='output_backward')
        # DEBUGGING
        ...
        attention_r = tf.reshape(tf.matmul(tf.reshape(tf.nn.softmax(
            tf.reshape(tf.matmul(tf.reshape(tf.tanh(output_h), [total_num * num_steps, gru_size]), attention_w),
                       [total_num, num_steps])), [total_num, 1, num_steps]), output_h), [total_num, gru_size], name='attention_r')
        ...
        for i in range(big_num):
            ...
            with tf.name_scope("loss"):
                self.loss.append(tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=sen_out[i], logits=self.input_y[i])))
                if i == 0:
